...
export function ChartMessage({ chartData, isLoading = false, error }: ChartMessageProps) {
  ...
  if (isLoading) {
    return (
      <div className={`rounded-lg border p-4 sm:p-6 mb-4 sm:mb-6 ${theme === "dark"
      ...
      >
        ...
        <span className={`text-lg font-medium ${theme === "dark" ? "text-treq-gray-300" : "text-treq-gray-700"
          }`}>
          Gerando gráfico...
        </span>
        <p className={`text-sm mt-2 ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-500"
          }`}>
          {chartData.title}
        </p>
      </div>
      ...
      <div className={`rounded-lg border p-4 sm:p-6 mb-4 sm:mb-6 ${theme === "dark"
        ...
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <div className={`h-5 w-5 rounded-full flex items-center justify-center ${theme === "dark" ? "bg-treq-error" : "bg-treq-error-light"
              }`}>
              <span className={`text-xs font-bold ${theme === "dark" ? "text-white" : "text-treq-error-dark"
                }`}>
                !
              </span>
            </div>
          ...
          <div className="ml-3">
            <p className={`font-medium ${theme === "dark" ? "text-treq-gray-200" : "text-treq-error-dark"
              }`}>
              Erro ao carregar gráfico
            </p>
            <p className={`mt-1 text-sm ${theme === "dark" ? "text-treq-gray-300" : "text-treq-error-dark"
              }`}>
    ...
    return (
      <div className={`rounded-lg border p-4 sm:p-6 mb-4 sm:mb-6 ${theme === "dark"
        ...
        <div className="text-center py-12">
          <div className={`inline-block p-3 rounded-full mb-4 ${theme === "dark"
          ...
          <h3 className={`text-lg font-medium mb-1 ${theme === "dark" ? "text-treq-gray-200" : "text-treq-gray-900"
            }`}>
            {chartData.metadata?.message || 'Nenhum dado disponível'}
          </h3>
          <p className={`text-sm ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-500"
            }`}>
  ...
  const MobileTooltipModal = () => {
    ...
    return (
      ...
      >
        <div
          className={`w-full max-w-sm rounded-lg shadow-xl border ${theme === "dark"
  ...
  const CustomPieTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div
        className={`px-2.5 py-2 rounded-md shadow-lg border z-50 ${theme === "dark"
          ? "bg-treq-gray-800 border-treq-gray-700 text-treq-gray-50"
          : "bg-white border-treq-gray-200 text-treq-gray-900"
          }`}
        style={{
  ...
  const CustomLegend = ({ payload }: any) => {
    ...
    return (
      <div className={`${isMobile ? 'mt-2 px-1.5' : 'mt-3 px-2'} ${theme === "dark" ? "text-treq-gray-300" : "text-treq-gray-700"
        }`}>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {severityDescription && (
                    <div className={`${isMobile ? 'text-[9px]' : 'text-xs'} mt-0.5 leading-relaxed ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-600"
                      }`}>
                      {severityDescription}
                    </div>
                  )}
  ...
  const CustomBarLineTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      ...
      >
          ...
          {payload.map((entry: any, index: number) => {
            ...
            return (
              <div key={index} className="flex items-center gap-2">
                ...
                <span className={`${isMobile ? 'text-[10px]' : 'text-xs'} font-semibold ${isMeta
        ...
        {description && (
          <div className={`mt-2 pt-2 border-t ${theme === "dark" ? "border-treq-gray-700" : "border-treq-gray-200"
            }`}>
            <p className={`${isMobile ? 'text-[10px]' : 'text-xs'} leading-relaxed ${theme === "dark" ? "text-treq-gray-300" : "text-treq-gray-700"
              }`}>
              {description}
            </p>
          </div>
    ...
    switch (chartData.type) {
      case "pie_chart":
        ...
        const pieOuterRadius = isMobile
          ? 45  // Mobile: altura 280px - 80px margens - 100px legenda = 100px disponível / 2 = 50px máximo
          : 110; // Desktop: altura 450px - 80px margens - 120px legenda = 250px disponível / 2 = 125px máximo
      ...
      case "bar_chart":
        ...
        const handleBarChartClick = (data: any, index?: any) => {
          if (!isMobile) return;
                  ...
                  if (dataset.type === "line") {
                    return (
                      <Line
                        ...
                        stroke={(dataset.borderColor || COLORS.treqSuccess) as string}
                        strokeWidth={2}
                        dot={{
                          fill: (dataset.borderColor || COLORS.treqSuccess) as string,
                          r: isMobile ? 3.5 : 5,
                          stroke: bgColor,
                          strokeWidth: 2
                        }}
                    ...
                    return (
                      <Bar
                        key={dataset.label}
                        dataKey={dataset.label}
                        fill={(dataset.backgroundColor || COLORS.treqYellow) as string}
                        stroke={(dataset.borderColor || COLORS.treqYellowDark) as string}
      ...
      case "line_chart":
        const handleLineChartClick = (data: any, index?: any) => {
          if (!isMobile) return;
              ...
              >
                ...
                {chartData.data.datasets.map((dataset, index) => (
                  <Line
                    ...
                    type="monotone"
                    dataKey={dataset.label}
                    stroke={(dataset.borderColor || COLORS.treqInfo) as string}
                    strokeWidth={2}
                    dot={{
                      fill: (dataset.borderColor || COLORS.treqInfo) as string,
                      r: isMobile ? 3.5 : 5,
                      stroke: bgColor,
                      strokeWidth: 2,
      ...
      default:
        return (
          <div className={`p-8 text-center ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-500"
            }`}>
  ...
  return (
    ...
    >
      ...
      <div className={`${isMobile ? 'px-2 py-1.5' : 'px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2.5'}`}>
        <h3
          className={`${isMobile ? 'text-xs' : 'text-sm sm:text-base md:text-lg'} font-semibold mb-0.5 ${theme === "dark" ? "text-treq-gray-50" : "text-treq-gray-900"
            }`}
        >
          {chartData.title}
        </h3>
        {chartData.subtitle && (
          <p
            className={`${isMobile ? 'text-[10px]' : 'text-xs sm:text-xs md:text-sm'} mb-1 ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-600"
              }`}
          >
            {chartData.subtitle}
          </p>
        ...
        {chartData.description && (
          <p
            className={`${isMobile ? 'text-[10px]' : 'text-xs sm:text-xs md:text-sm'} leading-relaxed ${theme === "dark" ? "text-treq-gray-400" : "text-treq-gray-600"
              }`}
            style={{ opacity: 0.85 }}
          >
            {chartData.description}
      ...
      {chartData.metadata && (
        <div
          className={`${isMobile ? 'px-2 py-1' : 'px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2'} border-t ${isMobile ? 'text-[9px]' : 'text-xs'} ${theme === "dark"
        ...
        >
          ...
          {chartData.metadata.alert_types && (
            <div className="mt-2 pt-2 border-t border-opacity-30">
              <p className={`font-semibold mb-1 ${theme === "dark" ? "text-treq-gray-300" : "text-treq-gray-700"
                }`}>
              ...
              <ul className="space-y-0.5">
                {Object.entries(chartData.metadata.severity_levels).map(([level, description]: [string, any]) => (
                  <li key={level} className="flex items-start gap-1.5">
                    <span className={`font-medium ${level === "Crítico" ? "text-red-500" :
